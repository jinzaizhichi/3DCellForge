...
3DCellForge 是一个 React + Three.js 原型，用于在浏览器里展示高质感的细胞 3D 场景。它支持 WebGL 拖拽旋转、滚轮缩放、左侧细胞类型 / 中央 3D 舞台 / 右侧工具区的工作台布局、截图、GLB 导出、历史上传默认收起、Demo 演示模式、生成任务中心，以及通过 Hyper3D / Tripo / Fal.ai / Hunyuan3D / JS Depth / 本地模型导入生成或加载 3D 模型。
...
- 细胞器详情抽屉、显微镜参考图、对比面板、生物学笔记、图库操作和生成任务中心。
...
- 右侧 `Generation Tasks` 可以查看上传、生成、导入状态，并对失败任务重试。
